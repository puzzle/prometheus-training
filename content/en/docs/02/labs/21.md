...
Once a virtual machine is defined in our CMDB, it is added to your monitoring stack using the defined business service. Let us check if you already have some machines monitored by your monitoring stack using the `managed="cmdb-service-asset"` selector and the `up`.
...
{{% alert title="Note" color="primary" %}}
Depending on your business service, the list may be empty. For reference, you can check the metric on a stack provided by the training manager.
{{% /alert %}}
