...
Additionally select your monitoring namespace using the `namespace` label. Replace "examples-monitoring" with the monitoring name you defined earlier in lab 01.
```promql
prometheus_http_requests_total{code="200", handler="/api/v1/query",namespace="examples-monitoring"}
```
{{% /onlyWhen %}}
...
{{% onlyWhen baloise %}}
```promql
prometheus_http_requests_total{code="200", handler="/api/v1/query",namespace="examples-monitoring"}[5m]
```
{{% /onlyWhen %}}
...
{{% onlyWhen baloise %}}
```promql
prometheus_http_requests_total{handler=~"/api.*", namespace="examples-monitoring"}
```
{{% /onlyWhen %}}
...
{{% onlyWhen baloise %}}
```promql
prometheus_http_requests_total{code!~"2..",namespace="examples-monitoring"}
```
{{% /onlyWhen %}}
...
Additionally select your Prometheus instance using the `prometheus` label. Replace `examples-monitoring/prometheus` with the monitoring name you defined earlier in lab 01.

```promql
sum(scrape_samples_scraped{prometheus="examples-monitoring/prometheus"})
```
{{% /onlyWhen %}}
...
{{% onlyWhen baloise %}}
{{% alert title="Note" color="primary" %}}
Select a specific node by filtering using the label `instance`. In our case and in the following examples, we used "bitbucket.balgroupit.com:9100" as a reference.
{{% /alert %}}
{{% /onlyWhen %}}
...
```promql
rate(
  node_cpu_seconds_total{instance="bitbucket.balgroupit.com:9100"}[5m]
  )
* 100
```
{{% /onlyWhen %}}
...
(
  1 -
  rate(
      node_cpu_seconds_total{mode="idle",instance="bitbucket.balgroupit.com:9100"}[5m]
      )
)
* 100
```
...
{{% onlyWhen baloise %}}
```promql
sum by(instance) (node_memory_MemAvailable_bytes{instance="bitbucket.balgroupit.com:9100"})
/
...
{{% onlyWhen baloise %}}
```promql
prometheus_http_request_duration_seconds_bucket{handler="/metrics",le="0.1",namespace="examples-monitoring"}
```
{{% /onlyWhenNot %}}
...
sum(
  rate(
    prometheus_http_request_duration_seconds_bucket{handler="/metrics",le="0.1",namespace="examples-monitoring"}[5m]
  )
) by (job, handler)
/
sum(
  rate(
    prometheus_http_request_duration_seconds_count{handler="/metrics",namespace="examples-monitoring"}[5m]
  )
) by (job, handler)
* 100
```
...
histogram_quantile(
  0.9,
  rate(
    prometheus_http_request_duration_seconds_bucket{handler="/metrics",namespace="examples-monitoring"}[5m]
  )
)
```
{{% /onlyWhenNot %}}
...
{{% onlyWhen baloise %}}
predict_linear(node_filesystem_avail_bytes{mountpoint="/",instance="bitbucket.balgroupit.com:9100"}[1h], 3600 * 4) < 0
{{% /onlyWhen %}}
...
{{% onlyWhen baloise %}}
predict_linear(node_filesystem_avail_bytes{mountpoint="/",instance="bitbucket.balgroupit.com:9100"}[1h], 3600 * 4)
{{% /onlyWhen %}}
...
(
  (
    time() - node_boot_time_seconds{instance="bitbucket.balgroupit.com:9100"}
  ) / 60
)
* on(instance) group_left(release) node_uname_info
```
...
{{% onlyWhen baloise %}}
```promql
node_uname_info{instance="bitbucket.balgroupit.com:9100"} * on(instance) group_right(release)
(
  (
    time() - node_boot_time_seconds
  ) / 60
