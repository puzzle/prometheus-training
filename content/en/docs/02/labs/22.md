...
Now let's reuse that query to create a query that returns the current CPU usage per core of the Prometheus server in %. The usage is the total (100%) [minus](https://prometheus.io/docs/prometheus/latest/querying/operators/#arithmetic-binary-operators) the CPU usage **idle**.
