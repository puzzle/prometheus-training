...
PromQL is the query language that let's you select, aggregate, filter the time series data, collected by prometheus in real time.

{{% alert title="Note" color="primary" %}}
PromQL can seem a bit overwhelming. It takes a bit of time to understand it.
{{% /alert %}}
...
The result represents all timeseries for the http requests sent to your prometheus Server.
...
The result represents all timeseries for the http requests sent to the query endpoint of your prometheus Server, which were successful `code="200"`.
...
A range vectore can not be graphed directly in the Prometheus UI, use the console view to display the result.

Using regular expressions, you could filter time series only for `handlers` whose name match a certain pattern, in this case, all `handlers` that start with `/api`:
...
To select all HTTP status codes except 2xx ones, you could run:
...
The [Prometheus Aggregation operators](https://prometheus.io/docs/prometheus/latest/querying/operators/#aggregation-operators) help us to aggregate timeseries in PromQL.

There is a prometheus metric that represents all scraped samples by Prometheus
...
The metric `scrape_samples_scraped` represents the total of scraped samples by `job` and `instance`.

To get the total amount of scraped samples we there fore use the [Prometheus Aggregation operators](https://prometheus.io/docs/prometheus/latest/querying/operators/#aggregation-operators) `sum` to sum sum values
...
Since the `rate` function calculates the per-second average of increase ot the time series in a **range vector**, we have to pass a range vector to the function `node_cpu_seconds_total{mode="idle"}[5m]`
...
In the previous lab we created a query, which returned the CPU **idle** usage.
Let's now reuse this query to create a query that returns the current CPU usage of the Prometheus server in %. The usage is all (100%) [minus](https://prometheus.io/docs/prometheus/latest/querying/operators/#arithmetic-binary-operators) the CPU usage **idle**
...
The `node exporter` exposes those two metrics:
...
We can simply divide the available memory time series with the total Memory of the node. And multiply it by 100 to get percent.
...
We can simply use the greater-than binary operator to compare the instant vector from the query to 20
...
As seen in previous labs http metrics for the prometheus server are available in the `prometheus` namespace.
