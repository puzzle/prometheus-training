...
Add another panel to the existing `happy_little_dashboard` with the panel name `Memory Available` that uses a variable `$disk`. Name the variable `disk` and label it `Select disk`. Display the metric `100 - ((node_filesystem_avail_bytes{mountpoint="$disk", instance="prometheus-training.balgroupit.com:9100"} * 100) / node_filesystem_size_bytes{mountpoint="$disk", instance="prometheus-training.balgroupit.com:9100"})` which can be used to calculate the free disk space of a certain mountpoint in percent, change the panel type to `Stat` and display it in %. Define the following thresholds:
...
* Frist we create the variable. Hit the little gear icon on the top right of the dashboard
  * Select **Variables** in the menu on the left **> Add Variable**
  * Set `Name` to `disk`
  * Choose **Query** in the dropdown menu for `Type`
