...
* Set the Prometheus volume size to `10Gi`
...
/dev/sda1                10.0G      8.4G      1.6G  84% /prometheus
```

{{% alert title="Note" color="primary" %}}
We use Minkube, which for demonstration purposes uses the `/dev/sda1` disk of your virtual machine as the storage backend for all Persistent Volumes. Therefore, you will always see a size of `10.0G` when checking from inside a container. If you use an appropriate storage backend, the size inside the container will correspond to the size of your Persistent Volume.
{{% /alert %}}
...
By default, the Prometheus operator stack will set the retention of your metrics to `24h`. As we have now 10Gi of disk space available, we can increase the retention.
...
{{< highlight yaml >}}{{< readfile file="content/en/docs/07/labs/mailcatcher.yml" >}}{{< /highlight >}}
