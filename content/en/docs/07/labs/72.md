...
{{% onlyWhen baloise %}}
---
title: "7.1 Tasks: OpenShift metrics"
weight: 7
sectionnumber: 7.2
---
...
**argocd:** Exposes metrics pushed to your pushgateway.
...
Navigate to the [Thanos Ruler user interface ](http://LOCALHOST:19090/rules) and take a look at the provided default Prometheus rules.
...
```promql
:kube_pod_container_status_restarts_total:offset * on(namespace, cluster) group_left(env) kube_namespace_labels{label_monitoringselector="examples-monitoring"} and on(pod, namespace) kube_pod_labels{label_app_kubernetes_io_part_of!="bdop"}
```
...
```promql
container_memory_working_set_bytes{namespace="examples-monitoring", pod=~"prometheus-prometheus-.*", container="prometheus"}
```
{{% /onlyWhen %}}
