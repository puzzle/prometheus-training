...
According to the [official Pushgateway documentation](https://github.com/prometheus/pushgateway/blob/master/README.md#command-line) you can delete either metrics for specific `jobs`, job - label combinations or all metrics.
...
It's also possible to specify labels to match, when deleting metrics:

```bash
curl -X DELETE http://localhost:9091/metrics/job/prometheus_training/instance/myinstance
```
